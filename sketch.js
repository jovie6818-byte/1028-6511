...
function drawQuestion() {
    ...
    textSize(22);
    fill(0);
    text(`Q: ${q.題目}`, 30, 80, width - 60, 100); // 限制題目文字範圍
...
function drawEndScreen() {
    ...
    textSize(30);
    fill(0);
    text(`最終得分：${score} / ${maxScore}`, width / 2, height / 2);
    
    textSize(24);
    if (score === maxScore) {
        fill(255, 165, 0); // 橘色
        text("💯 恭喜！您獲得滿分！知識淵博！", width / 2, height / 2 + 50);
    } else if (score >= maxScore * 0.6) {
        fill(0, 100, 150);
        text("👍 表現不錯！請再接再厲！", width / 2, height / 2 + 50);
    } else {
        fill(150, 0, 0);
        text("📚 沒關係，多複習一下 p5.js 函式！", width / 2, height / 2 + 50);
    }
